...
func normalizedTopUpDiscount(discount float64) float64 {
	if discount <= 0 {
		return 1.0
	}
	return discount
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	ProcessTopUpCommission(topUp.UserId, int(quota), normalizedTopUpDiscount(topUp.Discount), referenceId)
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		discount = normalizedTopUpDiscount(topUp.Discount)
		return nil
	})
...
func RechargeWaffo(tradeNo string, callerIp string) (err error) {
	...
	if quotaToAdd > 0 {
		...
		ProcessTopUpCommission(topUp.UserId, quotaToAdd, normalizedTopUpDiscount(topUp.Discount), tradeNo)
	}
