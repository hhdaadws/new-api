...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	topUp := &model.TopUp{
		...
		Amount:        req.Amount,
		Money:         chargedMoney,
		Discount:      getStripeAmountDiscount(float64(req.Amount)),
		TradeNo:       referenceId,
...
func getStripePayMoney(amount float64, group string) float64 {
	...
	discount := getStripeAmountDiscount(originalAmount)
	payMoney := amount * setting.StripeUnitPrice * topupGroupRatio * discount
	return payMoney
}

func getStripeAmountDiscount(originalAmount float64) float64 {
	discount := 1.0
