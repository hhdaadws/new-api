...
func getWaffoPayMoney(amount float64, group string) float64 {
	...
	discount := getWaffoAmountDiscount(originalAmount)
	return amount * setting.WaffoUnitPrice * topupGroupRatio * discount
}

func getWaffoAmountDiscount(originalAmount float64) float64 {
	discount := 1.0
...
func RequestWaffoPay(c *gin.Context) {
	...
	topUp := &model.TopUp{
		...
		Amount:        amount,
		Money:         payMoney,
		Discount:      getWaffoAmountDiscount(float64(req.Amount)),
